...
def predict_window(w, img, predictor):
    return predictor.predict(cv2.resize(img[w[0][1]:w[1][1], w[0][0]:w[1][0]], (64, 64)))
...
def find_cars(img, predictor, scale, overlap, ystart=400, ystop=650, xstart=0, xstop=1280):
    ...
    pix_per_cell = predictor.config['pix_per_cell']
    cell_per_block=predictor.config['cell_per_block']
    spatial_size=tuple(predictor.config['spatial_size'])
    hist_bins=predictor.config['hist_bins']
    X_scaler = predictor.scaler
    ...
    cells_per_step = max(int((1-overlap) * 8), 1)  # Instead of overlap, define how many cells to step
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            test_prediction = predictor.predict(subimg, precomputed_hog_features = hog_features)
            #test_prediction = predictor.predict(subimg)
            #if test_prediction != test_prediction2:
            #    raise Exception
...
def search_windows_parallel(img, windows, predictor):
    #1) Create an empty list to receive positive detection windows
    on_windows = []
    num_cores = multiprocessing.cpu_count()
    predictions = Parallel(n_jobs=num_cores)(delayed(predict_window)(w,img, predictor) for w in windows)
    indices = [i for i, p in enumerate(predictions) if p==1]
    on_windows = [windows[i] for i in indices]

    return on_windows


def search_windows(img, windows, predictor):
    #1) Create an empty list to receive positive detection windows
    on_windows = []
